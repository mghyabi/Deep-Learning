    ...
    def calc_gradient(self, X_train: np.ndarray, y_train: np.ndarray) -> np.ndarray:
        ...
        Returns:
            the gradient with respect to weights w; an array of the same shape
                as w
        """
        # TODO: ****this part is merged with the train function****
        return
...
#                 g1 = (-self.alpha * self.reg_const / BatchSize) * self.w
                g1 = (-self.alpha * self.reg_const / X_train.shape[0]) * self.w
                ...
                for i in BatchIndex:
                    ...
                    if np.sum(f[y] - f < 1) > 1:
                        ...
                        g2 = -self.alpha * np.tile(1*(f[y] - f < 1), [3072+1,1]).T * x #bias
                        g2[y] = -g2[y]
