...
#         hidden = None
        ...
        output, hidden = self.rnn(encoded.view(1, inp.size(0), -1), hidden)
        output = self.dropout(output)
        for i in range(self.n_layers - 1):
            output, hidden = self.rnn(output.view(1, inp.size(0), -1), hidden)
            output = self.dropout(output)
        output = self.decoder(output.view(inp.size(0), -1))
        output = self.dropout(output)
